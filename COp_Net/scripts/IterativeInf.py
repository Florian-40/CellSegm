...
def Inference(path_to_input_probamap , path_to_output_folder, cv_threshold = 0.1, max_iter = 50, device = 'cuda') : 

    """
    path_to_input_probamap : path to the input probamap, the file must be in .nii.gz format
    path_to_output_folder : path to the output folder where the final output will be saved
    device : device to use for the nnUNetv2 inference, (GPU), 'cpu' (CPU) and 'mps' (Apple M1/M2)
    """
    ...
    while i <= max_iter and cv_criterion[-1] > cv_threshold : 
        ...
        len(glob.glob(os.path.join(output_path,'CopNet_'+str(i),'*.npz'))) != len(glob.glob(os.path.join(input_path,'CopNet_'+str(i),'*.nii.gz'))):
            ...
            subprocess.run(["nnUNetv2_predict", "-i", CopNet_input_folder, "-o", CopNet_output_folder, "-d", "Dataset002_Real_FT_BALPubli", "-c", "2d", "-f", "0", "--save_probabilities", '-device' , device])
